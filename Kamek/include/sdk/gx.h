 ...
 *
 * \details This structure contains precompiled register state setting commands and data. The application must use the GXInitTexObj*()
 ...
 *
 * \details This structure contains precompiled register state setting commands and data. The application must use the GXInitTlutObj()
 * function to initialize or change this object. The proper size of the object is returned by
 *
 ...
 * GXInitTexCacheRegion() function to initialize or change this object. The proper size of the object is returned by
 * 
 ...
 *
 * \details This structure contains precompiled register state setting commands and data. The application must use the GXInitTlutRegion()
 * function to initialize or change this object. The proper size of the object is returned by
 *
 ...
 *
 * \details This structure contains precompiled register state setting commands and data. The application must use the GXInitLight*() functions
 ...
 *
 * \details This structure contains precompiled register state setting commands and data. The application must use the GXInitFifo*() functions
 * to initialize or change this object. The proper size of the object is returned by
 *
...
/*!
 * \fn GXFifoObj* GXInit(void *base,u32 size)
 ...
 * is the size of the FIFO in bytes and must be a multiple of 32B. Refer to additional notes in GXInitFifoBase() concerning
 * the FIFO memory.
 *
 * \note It is also possible to override the default immediate-mode style and instead buffer the graphics for frame <i>n+1</i>
 * while the GP is reading the graphics for frame <i>n</i>. See GXSetCPUFifo() and GXSetGPFifo() for further information.<br><br>
 *
 ...
 * The current GX thread can be changed by calling GXSetCurrentGXThread().
 *
...
/*!
 * \fn void GXInitFifoBase(GXFifoObj *fifo,void *base,u32 size)
 ...
 *
 * \note GXInit() also takes the arguments \a base and \a size and initializes a FIFO using these values and attaches the FIFO
 * to both the CPU and GP. The application must allocate the memory for the graphics FIFO before calling GXInit(). Therefore, it
 * is not necessary to call this function unless you want to resize the default FIFO sometime after GXInit() has been called or
 ...
 * necessary to call GXInitFifoPtrs() when initializing the FIFO. In addition, This function sets the FIFO's high water mark to
 * (size-16kB) and the low water mark to (size/2), so it is also not necessary to call GXInitFifoLimits().
 *
...
/*!
 * \fn void GXInitFifoLimits(GXFifoObj *fifo,u32 hiwatermark,u32 lowatermark)
 ...
 * Graphics Processor (GP) (see GXSetCPUFifo() and GXSetGPFifo()).
 *
...
/*!
 * \fn void GXInitFifoPtrs(GXFifoObj *fifo,void *rd_ptr,void *wt_ptr)
...
/*!
 * \fn void GXGetFifoPtrs(GXFifoObj *fifo,void **rd_ptr,void **wt_ptr)
 * \brief Returns the current value of the Graphics FIFO read and write pointers.
 *
 * \note See GXEnableBreakPt() for an example of why you would do this.
 *
 ...
 * \return none
 */
void GXGetFifoPtrs(GXFifoObj *fifo,void **rd_ptr,void **wt_ptr);

/*!
 * \fn void GXSetCPUFifo(GXFifoObj *fifo)
...
/*!
 * \fn void GXSetGPFifo(GXFifoObj *fifo)
 ...
 * the \a cmdIdle status returned by GXGetGPStatus().<br><br>
 *
 * \note The break point mechanism can be used to force the FIFO to stop reading commands at a certain point; see GXEnableBreakPt().
 *
...
/*!
 * \fn void GXGetCPUFifo(GXFifoObj *fifo)
...
/*!
 * \fn void GXGetGPFifo(GXFifoObj *fifo)
...
/*!
 * \fn void* GXGetFifoBase(GXFifoObj *fifo)
...
/*!
 * \fn u32 GXGetFifoCount(GXFifoObj *fifo)
 ...
 */
u32 GXGetFifoCount(GXFifoObj *fifo);

/*!
 * \fn u32 GXGetFifoSize(GXFifoObj *fifo)
...
/*!
 * \fn u8 GXGetFifoWrap(GXFifoObj *fifo)
 ...
 * \return wrap value
 */
u8 GXGetFifoWrap(GXFifoObj *fifo);

/*!
 * \fn GXDrawDoneCallback GXSetDrawDoneCallback(GXDrawDoneCallback cb)
 * \brief Installs a callback that is invoked whenever a DrawDone command is encountered by the GP.
 *
 * \details The DrawDone command is sent by GXSetDrawDone().
 *
...
/*!
 * \fn GXBreakPtCallback GXSetBreakPtCallback(GXBreakPtCallback cb)
 ...
 * GXPreloadEntireTexture() or TLUTs using GXLoadTlut(), you must make sure the command completed. You can use the draw sync mechanism to
 * do this; see GXSetDrawSync() and GXGetDrawSync().
 *
 * \return none
 */
void GXAbortFrame();
...
/*!
 * \fn void GXSetMisc(u32 token,u32 value)
 ...
 * GXSetDrawDoneCallback(), or you can poll the status bit with GXWaitDrawDone(). This function also flushes the write-gather
 * FIFO in the CPU to make sure that all commands are sent to the graphics processor.
 *
 * \note This function is normally used in multibuffer mode (see GXSetCPUFifo()). In immediate mode, the GXDrawDone() command
 ...
 * the frame buffer. You may want to execute some non-graphics operations between executing GXSetDrawDone() and this function, but
 * if you simply want to wait and have nothing to execute, you can use GXDrawDone().
 *
 * \note This function is normally used in immediate mode (see GXSetCPUFifo()). In multibuffer mode, sending the '<i>done</i>' command is
 * separated from polling the '<i>done</i>' status (see GXSetDrawDone() and GXWaitDrawDone()).
 *
 * \return none
 */
void GXWaitDrawDone();
...
/*!
 * \fn u16 GXGetDrawSync()
 * \brief Returns the value of the token register, which is written using the GXSetDrawSync() function.
 *
...
/*!
 * \fn void GXSetDrawSync(u16 token)
 ...
 *          with GXSetDrawSyncCallback(). Draw syncs can be used to notify the CPU that the graphics processor is finished using a shared
...
/*!
 * \fn GXDrawSyncCallback GXSetDrawSyncCallback(GXDrawSyncCallback cb)
...
/*!
 * \fn void GXDisableBreakPt()
 * \brief Allows reads from the FIFO currently attached to the Graphics Processor (GP) to resume.
 *
 * \details See GXEnableBreakPt() for an explanation of the FIFO break point feature.
 *
 * \note The breakpoint applies to the FIFO currently attached to the Graphics Processor (GP) (see GXSetGPFifo()).
 *
 * \return none
 */
...
/*!
 * \fn void GXEnableBreakPt(void *break_pt)
 ...
 * writing the graphics commands for one frame and are ready to start on the next. First, execute a GXFlush() command to make
 ...
 * boundary. Next, read the value of the current write pointer using GXGetFifoPtrs(). Write the value of the write pointer as
 * the break point address using GXEnableBreakPt(). When the FIFO read pointer reaches the break point address the hardware
 * will disable reads from the FIFO.  The status \a brkpt, returned by GXGetGPStatus(), can be polled to detect when the break point
 * is reached. The application can then decide when to disable the break point, using GXDisableBreakPt(), which will allow the FIFO
 ...
 * the GP FIFO, use GXDisableBreakPt().<br><br>
 *
 * \note Use GXSetBreakPtCallback() to set what function runs when the breakpoint is encountered.
 *
 ...
 *
 * \note This function is equivalent to calling GXSetDrawDone() then GXWaitDrawDone().
 *
 * \return none
 */
void GXDrawDone();
 ...
 * vertex descriptor (see GXSetVtxDesc()) is indexed. Direct data bypasses the vertex cache. Direct data is any attribute that is set to
...
/*!
 * \fn void GXClearVtxDesc()
 * \brief Clears all vertex attributes of the current vertex descriptor to GX_NONE.
 *
 * \note The same functionality can be obtained using GXSetVtxDescv(), however using GXClearVtxDesc() is much more efficient.
 *
 * \return none
 */
void GXClearVtxDesc();
...
/*!
 * \fn void GXSetProjection(Mtx44 mt,u8 type)
...
/*!
 * \fn void GXSetViewport(f32 xOrig,f32 yOrig,f32 wd,f32 ht,f32 nearZ,f32 farZ)
 ...
 * viewport to be adjusted by 1/2 line for interlaced field rendering modes; see GXSetViewportJitter(). The viewport depth parameters are normalized coordinates
 ...
 * you need to shift the viewport up or left, consider using GXSetScissorBoxOffset() instead.
 *
...
/*!
 * \fn void GXSetViewportJitter(f32 xOrig,f32 yOrig,f32 wd,f32 ht,f32 nearZ,f32 farZ,u32 field)
 ...
 *
 * \note To set a viewport <i>without</i> jitter, use GXSetViewport().
 *
 * \param[in] xOrig left-most X coordinate on the screen
 * \param[in] yOrig top-most Y coordinate on the screen
 * \param[in] wd width of the viewport
...
/*!
 * \fn void GXSetChanCtrl(s32 channel,u8 enable,u8 ambsrc,u8 matsrc,u8 litmask,u8 diff_fn,u8 attn_fn)
 ...
 * controls (i.e., the lighting equation). GXInit() sets the \a enable for all channels to <tt>GX_FALSE</tt>. This function only configures the lighting
 * channel; to output the result of the channel computation, use GXSetNumChans().
 *
 ...
 *
 * \note When \a ambsrc is set to <tt>GX_SRC_REG</tt>, the color set by GXSetChanAmbColor() is used as the ambient color. When \a matsrc is <tt>GX_SRC_REG</tt>, the color set
 * by GXSetChanMatColor() is used as the material color.
 *
 ...
 *
 * \details This color will be used by the channel as the ambient color if the ambient source, set by GXSetChanCtrl(), is <tt>GX_SRC_REG</tt>.
 *
...
/*!
 * \fn void GXSetChanMatColor(s32 channel,GXColor color)
 * \brief Sets the material color register for color channel \a chan.
 *
 * \details This color will be used by the channel as the material color if the material source, set by GXSetChanCtrl(), is <tt>GX_SRC_REG</tt>.
 *
 * \param[in] channel channel to set
 * \param[in] color color to set it to
 *
...
/*!
 * \fn void GXSetArray(u32 attr,void *ptr,u8 stride)
 ...
 *
 * When drawing a graphics primitive that has been enabled to use indexed attributes (see GXSetVtxDesc()), <i>attr_idx</i> is supplied in the vertex
 * data. The format and size of the data in the array must also be described using GXSetVtxAttrFmt(). You can also index other data, such as
 * matrices (see GXLoadPosMtxIdx(), GXLoadNrmMtxIdx3x3(), and GXLoadTexMtxIdx()), and light objects (see GXLoadLightObjIdx()). In the case
...
/*!
 * \fn void GXSetVtxAttrFmt(u8 vtxfmt,u32 vtxattr,u32 comptype,u32 compsize,u32 frac)
 ...
 * all eight vertex formats and then select one of them during the actual drawing of geometry (See GXBegin()). Note that all vertices used to draw a
 ...
 * using GXSetVtxDesc(), completely define the vertex data format.
 *
 ...
 *
 * \note The constant <tt>GX_MAX_VTXATTRFMT_LISTSIZE</tt> should be used to allocate the list. You can get a current vertex format using GXGetVtxAttrFmtv().
 *
 * \param[in] vtxfmt \ref vtxfmt
...
/*!
 * \fn void GXSetVtxDesc(u8 attr,u8 type)
...
/*!
 * \fn void GXSetVtxDescv(GXVtxDesc *attr_list)
...
/*!
 * \fn void GXGetVtxDescv(GXVtxDesc *attr_list)
 * \brief Gets the type of multiple attributes.
 *
 * \details This function saves the attributes that are current set. This is usually used in conjunction with GXSetVtxDescv
 *
...
/*!
 * \fn u32 GXEndDispList()
 * \brief Ends a display list and resumes writing graphics commands to the CPU FIFO.
 *
 * \details This function returns the size of the display list written to the display list buffer since GXBeginDispList() was called. If
 ...
 * GXCallDispList() when the display list needs to be executed.
 *
 ...
 * at least 32 bytes larger than the maximum expected amount of data stored. This function calls GXFlush(), and thus it is not necessary
 * to call GXFlush() explicitly after creating the display list.<br><br>
 *
 * \note A display list cannot be nested; i.e., no display list functions (GXBeginDispList(), GXEndDispList() and GXCallDispList()) can
 * be called between a GXBeginDispList() and GXEndDispList() pair.<br><br>
 *
 * \note To execute a display list, use GXCallDispList().
 *
 * \return 0 if display list size exceeds buffer, otherwise gives list size in bytes
 *
 * \bug Specifying a display list buffer size for GXBeginDispList() the exact size that the display list will be (after padding) will cause
 * this function to return a very large (and very incorrect) value.
 */
u32 GXEndDispList();

/*!
 * \fn void GXBegin(u8 primitve,u8 vtxfmt,u16 vtxcnt)
 * \brief Begins drawing of a graphics primitive.
 *
 * \details To draw a graphics primitive, a stream of vertex data matching the description of both GXSetVtxDesc() and GXSetVtxAttrFmt() is
 * enclosed between GXBegin()/GXEnd() pairs. The number of vertices between GXBegin() and GXEnd() must match that specified by the \a vtxcnt
...
/*!
 * \fn void GXBeginDispList(void *list,u32 size)
 ...
 * display list buffer instead of the FIFO until GXEndDispList() is called. Writes to the CPU FIFO will be re-enabled when the function
 * GXEndDispList() executes.
 *
 ...
 * behavior or even graphics pipeline hang. The most recommended safe way is putting only primitives (regions enclosed by GXBegin() and
 * GXEnd()) that don't cause any state collisions.
 *
 ...
 * of the buffer, GXEndDispList() will return 0. The address of the buffer must be 32-byte aligned; memalign() can return 32-byte-aligned
...
/*!
 * \fn void GXCallDispList(void *list,u32 nbytes)
 ...
 * GXBeginDispList()/GXEndDispList(), this padding will be inserted automatically.
 *
 * \param[in] list 32-byte aligned pointer to the display list buffer
 * \param[in] nbytes number of bytes in the display list. Use the return value of GXEndDispList() here.
 *
 * \return none
 */
...
static inline void GXTexCoord1f32(f32 s)
{
	wgPipe.F32 = s;
}

static inline void GXTexCoord1u16(u16 s)
{
	wgPipe.U16 = s;
}
...
static inline void GXTexCoord1u8(u8 s)
{
	wgPipe.U8 = s;
}

static inline void GXTexCoord1s8(s8 s)
{
	wgPipe.S8 = s;
}
...
/*!
 * \fn void GXAdjustForOverscan(GXRModeObj *rmin,GXRModeObj *rmout,u16 hor,u16 ver)
 ...
 * \returns none
 */
void GXAdjustForOverscan(GXRModeObj *rmin,GXRModeObj *rmout,u16 hor,u16 ver);
 ...
 * GXSetCurrentMtx()), or by setting a matrix \a pnidx for each vertex. The parameter \a mt is a pointer to a 3x4 (row x column) matrix. The
 * parameter \a pnidx is used to refer to the matrix location (see \ref pnmtx) in matrix memory.
 *
 * You can also load a normal matrix (GXLoadNrmMtxImm() or GXLoadNrmMtxIdx3x3()) to the same \a pnidx. Generally, the normal matrix
...
/*!
 * \fn void GXLoadPosMtxIdx(u16 mtxidx,u32 pnidx)
 * \brief Loads a 3x4 modelview matrix at index \a mtxidx from the array in main memory.
 *
 * \details The array is set by GXSetArray(), and the matrix is loaded into matrix memory at index \a pnidx (see \ref pnmtx). This
 ...
 * GXSetCurrentMtx()) or by setting a matrix \a pnidx for each vertex (see GXSetVtxDesc()). The matrix will be loaded through the vertex cache.
 *
 * You can also load a normal matrix (GXLoadNrmMtxImm() or GXLoadNrmMtxIdx3x3()) to the same \a pnidx. Generally, the normal matrix
 * will be the inverse transpose of the position matrix. The normal matrix is used during vertex lighting. In cases where the modelview
 * and inverse transpose of the modelview (excluding translation) are the same, you can load the <i>same</i> matrix for both position and normal
...
/*!
 * \fn void GXLoadNrmMtxImm(Mtx mt,u32 pnidx)
 * \brief Used to load a normal transform matrix into matrix memory at location \a pnidx from the 4x3 matrix \a mt.
 *
 * \details This matrix is used to transform normals in model space to view space, either by making it the current matrix (see GXSetCurrentMtx()),
 ...
 *
 * \note You can also load a position matrix (GXLoadPosMtxImm()) to the same \a pnidx. Normally, the normal matrix will be the inverse transpose of
...
/*!
 * \fn void GXLoadNrmMtxIdx3x3(u16 mtxidx,u32 pnidx)
 ...
 *
 * \details The array is set by set by GXSetArray(), and the matrix is loaded into matrix memory at index \a pnidx. This matrix can be used to
 * transform normals in model space to view space, either by making the matrix the current one (see GXSetCurrentMtx()), or by setting a
 * matrix \a pnidx for each vertex (see GXSetVtxDesc()). The matrix will be loaded through the vertex cache. You can also load a position
 * matrix (GXLoadPosMtxImm() or GXLoadPosMtxIdx()) to the same \a pnidx.
 *
 * \note You cannot use an indexed load to load a 3x3 matrix from an indexed 3x4 matrix in main memory; you must use GXLoadNrmMtxImm() for
 * this case.<br><br>
 *
...
/*!
 * \fn void GXLoadTexMtxImm(Mtx mt,u32 texidx,u8 type)
 ...
 * coordinates are used for projected textures, reflection mapping, etc. See GXSetTexCoordGen() for more details.
 *
 ...
 * matrix is preloaded by GXInit().<br><br>
 *
 ...
 * GXSetTexCoordGen2() for information about how to use post-transform texture matrices.
 *
 * \param[in] mt the matrix to load
...
/*!
 * \fn void GXLoadTexMtxIdx(u16 mtxidx,u32 texidx,u8 type)
 * \brief Loads a texture matrix at index \a mtxidx from the array in main memory
 *
 * \details The array is set by GXSetArray(), and the matrix is loaded into matrix memory at location \a texid. The loaded matrix can be
 ...
 * texture coordinates (see GXSetTexCoordGen()). The matrix is loaded through the vertex cache. The size of the matrix to load is indicated by its
...
/*!
 * \fn void GXSetCurrentMtx(u32 mtx)
 ...
 * as long as a matrix index is not present in the vertex data (see GXSetVtxDesc()). If the current vertex descriptor enables <tt>GX_VA_PTNMTXIDX</tt>,
...
/*!
 * \fn void GXSetTevOp(u8 tevstage,u8 mode)
 ...
 * GXSetTevColorIn(), GXSetTevColorOp(), GXSetTevAlphaIn() and GXSetTevAlphaOp() with predefined arguments to implement familiar texture
 * combining functions.
 *
 * \note To enable a consecutive set of TEV stages, you must call GXSetNumTevStages().
 *
...
/*!
 * \fn void GXSetTevColor(u8 tev_regid,GXColor color)
 ...
 *
 * \note This function can only set unsigned 8-bit colors. To set signed, 10-bit colors use GXSetTevColorS10().
 *
 ...
 * \return none
 */
void GXSetTevColor(u8 tev_regid,GXColor color);

/*!
 * \fn void GXSetTevColorS10(u8 tev_regid,GXColorS10 color)
 ...
 *
 * \note This function enables the color components to be signed 10-bit numbers. To set 8-bit unsigned colors (the common case), use GXSetTevColor().
 *
 * \param[in] tev_regid \ref tevcoloutreg.
 ...
 * \return none
 */
void GXSetTevColorS10(u8 tev_regid,GXColorS10 color);

/*!
 * \fn void GXSetTevColorIn(u8 tevstage,u8 a,u8 b,u8 c,u8 d)
...
/*!
 * \fn void GXSetTevAlphaIn(u8 tevstage,u8 a,u8 b,u8 c,u8 d)
...
/*!
 * \fn void GXSetTevColorOp(u8 tevstage,u8 tevop,u8 tevbias,u8 tevscale,u8 clamp,u8 tevregid)
 ...
 *
 *
 * The input sources a,b,c and d are set using GXSetTevColorIn().
 *
 * \param[in] tevstage \ref tevstage.
...
/*!
 * \fn void GXSetTevAlphaOp(u8 tevstage,u8 tevop,u8 tevbias,u8 tevscale,u8 clamp,u8 tevregid)
 ...
 *        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;\a tevregid = (d (\a tevop) ((1.0 - c)*a + c*b) + \a tevbias) * \a tevscale;<br><br>
 *
 * The input sources a,b,c and d are set using GXSetTevAlphaIn().
 *
 * \param[in] tevstage \ref tevstage.
 * \param[in] tevop \ref tevop
 * \param[in] tevbias \ref tevbias.
 ...
 *
 * \details Texture coordinates are generated from input data as described by GXSetTexCoordGen(). The generated texture coordinates are linked to
 * specific textures and specific TEV stages using GXSetTevOrder().
 *
 ...
 * If \a nr is set to 0, no texture coordinates will be generated. In this case, at least one color channel must be output (see GXSetNumChans()).
 *
...
/*!
 * \fn void GXSetTexCoordGen(u16 texcoord,u32 tgen_typ,u32 tgen_src,u32 mtxsrc)
 ...
 *
 * The current vertex descriptor as set by GXSetVtxDesc() only describes the data input to the graphics processor. Using this function, you can create
 ...
 * coordinate can be bound to a texture using GXSetTevOrder(). GXSetNumTexGens() specifies a consecutive number of texture coordinates, starting at
 * <tt>GX_TEXCOORD0</tt>, that are available to GXSetTevOrder().
 *
 ...
 * needs. Keep in mind, however, that modelview matrices (see GXLoadPosMtxImm() and \ref pnmtx) and texture matrices share matrix memory.
 *
 * \note Input texture coordinates must always go through the texture coordinate generation hardware. GXInit() initializes the hardware (by calling
...
/*!
 * \fn void GXSetTexCoordGen2(u16 texcoord,u32 tgen_typ,u32 tgen_src,u32 mtxsrc,u32 normalize,u32 postmtx)
 * \brief An extension of GXSetTexCoordGen(). It allows one to specify additional texgen options.
 *
 * \details The first four arguments are identical to those for GXSetTexCoordGen() and function in the same way. All requirements for the first
 ...
 * \return none
 */
void GXSetTexCoordGen2(u16 texcoord,u32 tgen_typ,u32 tgen_src,u32 mtxsrc,u32 normalize,u32 postmtx);
 ...
 *
 * Z textures are always the output from the last active Texture Environment (TEV) stage (see GXSetNumTevStages()) when enabled. When Z texturing is
 ...
 * 
 * \note GXInit() disables Z texturing.
 *
...
/*!
 * \fn void GXSetZMode(u8 enable,u8 func,u8 update_enable)
 ...
 * parameter also affects whether the Z buffer is cleared during copy operations (see GXCopyDisp() and GXCopyTex()).
 *
...
/*!
 * \fn void GXSetZCompLoc(u8 before_tex)
 ...
 * visible; however, when alpha compare is used, Z buffering must be done after texturing (see GXSetAlphaCompare()).
 *
...
/*!
 * \fn void GXSetLineWidth(u8 width,u8 fmt)
 ...
 * values are added after the texture coordinate generation operation; see GXSetTexCoordGen().
 *
...
/*!
 * \fn void GXSetPointSize(u8 width,u8 fmt)
 ...
 * \a fmts are added after the texture coordinate generation operation; see GXSetTexCoordGen().
 *
...
/*!
 * \fn void GXSetBlendMode(u8 type,u8 src_fact,u8 dst_fact,u8 op)
 ...
 *
 * The <tt>GX_BL_DSTALPHA</tt> / <tt>GX_BL_INVDSTALPHA</tt> can be used only when the EFB has <tt>GX_PF_RGBA6_Z24</tt> as the pixel format (see GXSetPixelFmt()). If the pixel
 ...
 *
 * \note Color updates should be enabled by calling GXSetColorUpdate().
 *
 ...
 *
 * \note GXInit() sets this to <tt>GX_CULL_BACK</tt>.
 *
...
/*!
 * \fn void GXSetCoPlanar(u8 enable)
 ...
 *
 * \note The reference triangle can be culled using <tt>GXSetCullMode(GX_CULL_ALL)</tt> to create an invisible reference plane; however, the reference
 * triangle must not be completely out of view, i.e. trivially rejected by clipping.<br><br>
 *
 * \note GXInit() disables coplanar mode.
 *
...
/*!
 * \fn void GXEnableTexOffsets(u8 coord,u8 line_enable,u8 point_enable)
 * \brief Enables a special texture offset feature for points and lines.
 *
 * \details When a point's size is defined using GXSetPointSize() or a line's width is described using GXSetLineWidth(), you can also specify a second
 ...
 * point or line. The \a fmts are added after the texture coordinate generation operation; see GXSetTexCoordGen(). This function enables this
 ...
 *
 * \note GXInit() sets this to <tt>GX_CLIP_ENABLE</tt>.
 *
...
/*!
 * \fn void GXSetScissor(u32 xOrigin,u32 yOrigin,u32 wd,u32 ht)
 ...
 * repositioned within the EFB using GXSetScissorBoxOffset().
 *
 * \note By default, the scissor rectangle is set to match the viewport rectangle. GXInit() initializes the scissor rectangle to match the viewport
...
/*!
 * \fn void GXSetScissorBoxOffset(s32 xoffset,s32 yoffset)
 ...
 *
 * \note GXInit() initializes the scissor box offset to zero. Since the GP works on 2x2 regions of pixels, only even offsets are allowed.
 *
...
/*!
 * \fn void GXSetNumChans(u8 num)
 * \brief Sets the number of color channels that are output to the TEV stages.
 *
 * \details Color channels are the mechanism used to compute per-vertex lighting effects. Color channels are controlled using GXSetChanCtrl().
 * Color channels are linked to specific TEV stages using GXSetTevOrder().
 *
 ...
 * must be generated (see GXSetNumTexGens()). If \a num is set to 1, then channel <tt>GX_COLOR0A0</tt> will be rasterized. If \a num is set to 2 (the maximum
...
/*!
 * \fn void GXSetTevOrder(u8 tevstage,u8 texcoord,u32 texmap,u8 color)
 * \brief Specifies the texture and rasterized color that will be available as inputs to this TEV \a tevstage.
 *
 * The texture coordinate \a texcoord is generated from input attributes using the GXSetTexCoordGen() function and is used to look up the
 * texture map, previously loaded by GXLoadTexObj(). The \a color to rasterize for this \a tevstage is also specified. The color
 * is the result of per-vertex lighting which is controlled by GXSetChanCtrl().
 *
 * This function will scale the normalized texture coordinates produced by GXSetTexCoordGen() according to the size of the texture map in the
 ...
 *
 * \code GXSetTevOrder(GX_TEVSTAGE1, GX_TEXCOORD0, GX_TEXMAP3 | GX_TEXMAP_DISABLE, GX_COLORNULL); \endcode
 *
 * \details This will scale <tt>GX_TEXCOORD0</tt> using <tt>GX_TEXMAP3</tt> but disable the lookup of <tt>GX_TEXMAP3</tt>.
 *
 * \note This function does not enable the TEV stage. To enable a consecutive number of TEV stages, starting at stage <tt>GX_TEVSTAGE0</tt>, use GXSetNumTevStages().<br><br>
 *
 * \note The operation of each TEV stage is independent. The color operations are controlled by GXSetTevColorIn() and GXSetTevColorOp(). The alpha
 * operations are controlled by GXSetTevAlphaIn() and GXSetTevAlphaOp().<br><br>
 *
 * \note The number of texture coordinates available for all the active TEV stages is set using GXSetNumTexGens(). The number of color channels
 * available for all the active TEV stages is set using GXSetNumChans(). Active TEV stages should not reference more texture coordinates or colors
 ...
 * <tt>GX_ALPHA_BUMP</tt> or <tt>GX_ALPHA_BUMPN</tt>, you can use "Bump alpha" component from indirect texture unit as rasterized color input (see GXSetTevIndirect()
...
/*!
 * \fn void GXSetNumTevStages(u8 num)
 ...
 * see GXSetTevColorOp() and GXSetTevAlphaOp(). At least one TEV stage must be enabled. If a Z-texture is enabled, the Z texture must be looked up on
 * the last stage; see GXSetZTexture().
 *
 ...
 * coordinates available is set using GXSetNumTexGens(). The number of color channels available is set using GXSetNumChans().<br><br>
 *
 * \note GXInit() will set \a num to 1.
 *
...
/*!
 * \fn void GXSetAlphaCompare(u8 comp0,u8 ref0,u8 aop,u8 comp1,u8 ref1)
 ...
 *
 * \note The output alpha can be used in the blending equation (see GXSetBlendMode()) to control how source and destination (frame buffer)
 * pixels are combined.<br><br>
 *
 * \note The Z compare can occur either before or after texturing (see GXSetZCompLoc()). In the case where Z compare occurs before texturing, the Z is
 ...
 *
 * \note The number of active TEV stages is specified using GXSetNumTevStages().
 *
...
/*!
 * \fn void GXSetTevKColor(u8 sel, GXColor col)
 ...
 * \return none
 */
void GXSetTevKColor(u8 sel, GXColor col);

/*!
 * \fn void GXSetTevKColorSel(u8 tevstage,u8 sel)
...
/*!
 * \fn void GXSetTevKAlphaSel(u8 tevstage,u8 sel)
...
/*!
 * \fn void GXSetTevKColorS10(u8 sel, GXColorS10 col)
 ...
 *
 * \note This function takes 10-bit signed values as color values; use GXSetTevColor() to give 8-bit values.
 *
 ...
 * \return none
 */
void GXSetTevKColorS10(u8 sel, GXColorS10 col);

/*!
 * \fn void GXSetTevSwapMode(u8 tevstage,u8 ras_sel,u8 tex_sel)
...
/*!
 * \fn void GXSetTevSwapModeTable(u8 swapid,u8 r,u8 g,u8 b,u8 a)
...
/*!
 * \fn void GXSetTevIndirect(u8 tevstage,u8 indtexid,u8 format,u8 bias,u8 mtxid,u8 wrap_s,u8 wrap_t,u8 addprev,u8 utclod,u8 a)
 ...
 * \return none
 */
void GXSetTevIndirect(u8 tevstage,u8 indtexid,u8 format,u8 bias,u8 mtxid,u8 wrap_s,u8 wrap_t,u8 addprev,u8 utclod,u8 a);
...
/*!
 * \fn void GXSetNumIndStages(u8 nstages)
...
/*!
 * \fn void GXSetIndTexOrder(u8 indtexstage,u8 texcoord,u8 texmap)
...
/*!
 * \fn void GXSetIndTexCoordScale(u8 indtexid,u8 scale_s,u8 scale_t)
...
/*!
 * \fn void GXSetFog(u8 type,f32 startz,f32 endz,f32 nearz,f32 farz,GXColor col)
 * \brief Enables fog.
 *
 ...
 * Range is cosine corrected Z in the x-z plane (eye coordinates), but is not corrected in the y direction (see GXSetFogRangeAdj()). The parameters \a startz and
 ...
 *
 * \note GXInit() turns fog off by default.
 *
...
/*!
 * \fn void GXSetFogRangeAdj(u8 enable,u16 center,GXFogAdjTbl *table)
 ...
 *
 * \note GXInit() disables range adjustment.
 *
 * \sa GXInitFogAdjTable()
 *
...
/*!
 * \fn GXSetFogColor(GXColor color)
...
/*!
 * \fn void GXInitFogAdjTable(GXFogAdjTbl *table,u16 width,f32 projmtx[4][4])
 * \brief Generates the standard range adjustment table and puts the results into \a table.
 *
 * \details This table can be used by GXSetFogRangeAdj() to adjust the eye-space Z used for fog based upon the X position of the pixels being rendered.
...
/*!
 * \fn void GXSetIndTexMatrix(u8 indtexmtx,f32 offset_mtx[2][3],s8 scale_exp)
...
/*!
 * \fn void GXSetTevIndBumpXYZ(u8 tevstage,u8 indstage,u8 mtx_sel)
 * \brief Sets up an environment-mapped bump-mapped indirect lookup.
 *
 ...
 * map) used is geometry-specific. Thus there is a space/computation tradeoff between using this function and using GXSetTevIndBumpST().
 *
...
/*!
 * \fn void GXSetTevIndTile(u8 tevstage,u8 indtexid,u16 tilesize_x,u16 tilesize_y,u16 tilespacing_x,u16 tilespacing_y,u8 indtexfmt,u8 indtexmtx,u8 bias_sel,u8 alpha_sel)
 ...
 * size and the size of the indirect map. In order to set the proper texture coordinate scale, one must call GXSetTexCoordScaleManually(). One can also use
 * GXSetIndTexCoordScale() in order to use the same texcoord for the indirect stage as the regular TEV stage.
 *
 * \param[in] tevstage \ref tevstage that is being affected
 * \param[in] indtexid \ref indtexstage results to use with this TEV stage
 ...
 * \return none
 */
void GXSetTevIndTile(u8 tevstage,u8 indtexid,u16 tilesize_x,u16 tilesize_y,u16 tilespacing_x,u16 tilespacing_y,u8 indtexfmt,u8 indtexmtx,u8 bias_sel,u8 alpha_sel);
 ...
 *
 * \note This is only useful when the previous stage texture coordinates took more than one stage to compute, as is the case for GXSetTevIndBumpST().
 *
...
/*!
 * \fn void GXSetColorUpdate(u8 enable)
 * \brief Enables or disables color-buffer updates when rendering into the Embedded Frame Buffer (EFB).
 *
 * \note This function also affects whether the color buffer is cleared during copies; see GXCopyDisp() and GXCopyTex().
 *
...
/*!
 * \fn void GXSetAlphaUpdate(u8 enable)
 * \brief Enables or disables alpha-buffer updates of the Embedded Frame Buffer (EFB).
 *
 * \note This function also affects whether the alpha buffer is cleared during copy operations; see GXCopyDisp() and GXCopyTex().<br><br>
 *
 * \note The only EFB pixel format supporting an alpha buffer is <tt>GX_PF_RGBA6_Z24</tt>; see GXSetPixelFmt(). The alpha \a enable is ignored for non-alpha
 * pixel formats.
 *
...
/*!
 * \fn void GXSetPixelFmt(u8 pix_fmt,u8 z_fmt)
 ...
 *
 * When \a pix_fmt is set to <tt>GX_PF_RGB565_Z16</tt>, multi-sample antialiasing is enabled. In order to get proper results, one must also call GXSetCopyFilter().
 * The position of the subsamples and the antialiasing filter coefficients are set using GXSetCopyFilter(). When antialiasing, three 16b color/Z
...
/*!
 * \fn void GXSetDither(u8 dither)
 * \brief Enables or disables dithering.
 *
 * \details A 4x4 Bayer matrix is used for dithering.
 *
 * \note Only valid when the pixel format (see GXSetPixelFmt()) is either <tt>GX_PF_RGBA6_Z24</tt> or <tt>GX_PF_RGB565_Z16</tt>.<br><br>
 *
...
/*!
 * \fn void GXSetDstAlpha(u8 enable,u8 a)
 * \brief Sets a constant alpha value for writing to the Embedded Frame Buffer (EFB).
 *
 * \note To be effective, the EFB pixel type must have an alpha channel (see GXSetPixelFmt()). The alpha compare operation (see
 * GXSetAlphaCompare()) and blending operations (see GXSetBlendMode()) still use source alpha (output from the last TEV stage) but when
...
/*!
 * \fn void GXSetFieldMask(u8 even_mask,u8 odd_mask)
...
/*!
 * \fn void GXSetFieldMode(u8 field_mode,u8 half_aspect_ratio)
 ...
 *
 * \note GXInit() sets both fields according to the default render mode.<br><br>
 *
...
/*!
 * \fn f32 GXGetYScaleFactor(u16 efbHeight,u16 xfbHeight)
 * \brief Calculates an appropriate Y scale factor value for GXSetDispCopyYScale() based on the height of the EFB and
 ...
 *
 * \return Y scale factor which can be used as argument of GXSetDispCopyYScale().
 */
f32 GXGetYScaleFactor(u16 efbHeight,u16 xfbHeight);

/*!
 * \fn u32 GXSetDispCopyYScale(f32 yscale)
 ...
 * have to call GXSetDispCopySrc() prior to this function call if you want to get the number of lines by using this function.
 *
...
/*!
 * \fn void GXSetDispCopySrc(u16 left,u16 top,u16 wd,u16 ht)
...
/*!
 * \fn void GXSetDispCopyDst(u16 wd,u16 ht)
...
/*!
 * \fn void GXSetCopyClamp(u8 clamp)
...
/*!
 * \fn void GXSetDispCopyGamma(u8 gamma)
...
/*!
 * \fn void GXSetCopyFilter(u8 aa,u8 sample_pattern[12][2],u8 vf,u8 vfilter[7])
 ...
 * GXSetCopyFilter(rmode->aa,rmode->sample_pattern,GX_TRUE,rmode->vfilter); \endcode
 *
 ...
 * <tt>GX_PF_RGB565_Z16</tt>; see GXSetPixelFmt().
 *
...
/*!
 * \fn void GXSetDispCopyFrame2Field(u8 mode)
 * \brief Determines which lines are read from the Embedded Frame Buffer (EFB) when using GXCopyDisp().
 *
 * \details Specifically, it determines whether all lines, only even lines, or only odd lines are read.
 *
 * \note The opposite function, which determines whether all lines, only even lines or only odd lines are <i>written</i> to the EFB, is GXSetFieldMask().<br><br>
 *
 * \note Only applies to display copies, GXCopyTex() always uses the <tt>GX_COPY_PROGRESSIVE</tt> mode.
 *
...
/*!
 * \fn void GXSetCopyClear(GXColor color,u32 zvalue)
...
/*!
 * \fn void GXCopyDisp(void *dest,u8 clear)
 * \brief Copies the embedded framebuffer (EFB) to the external framebuffer(XFB) in main memory.
 *
 * \note The stride of the XFB is set using GXSetDispCopyDst(). The source image in the EFB is described using GXSetDispCopySrc().<br><br>
 *
 ...
 * values set using GXSetCopyClear().
 *
...
/*!
 * \fn void GXSetTexCopySrc(u16 left,u16 top,u16 wd,u16 ht)
 * \brief Sets the source parameters for the Embedded Frame Buffer (EFB) to texture image copy.
 *
 * \details The GP will copy textures into the tiled texture format specified in GXCopyTex(). The GP always copies tiles (32B) so image widths and
...
/*!
 * \fn void GXSetTexCopyDst(u16 wd,u16 ht,u32 fmt,u8 mipmap)
...
/*!
 * \fn void GXCopyTex(void *dest,u8 clear)
 ...
 * to the current color(see GXSetCopyClear()) during the copy operation.
 *
 ...
 * flush before continuing. This command is useful in certain situation such as after using GXCopyTex() and before a primitive that uses the copied texture.
 *
...
/*!
 * \fn GXPokeAlphaMode(u8 func,u8 threshold)
 * \brief Sets a threshold which is compared to the alpha of pixels written to the Embedded Frame Buffer (EFB) using the GXPoke*() functions.
 *
 ...
 * true, the source color will be written to the EFB based on the result of the Z compare (see GXPokeZMode()). If the alpha compare function is false, the source
 * color is not written to the EFB.<br><br>
 *
 * \note The alpha compare test happens before the Z compare and before blending (see GXPokeBlendMode()).
 *
...
/*!
 * \fn void GXPokeAlphaUpdate(u8 update_enable)
 * \brief Enables or disables alpha-buffer updates for GXPoke*() functions.
 *
 * \details The normal rendering state (set by GXSetAlphaUpdate()) is not affected.
 *
...
/*!
 * \fn void GXPokeColorUpdate(u8 update_enable)
 * \brief Enables or disables color-buffer updates when writing the Embedded Frame Buffer (EFB) using the GXPoke*() functions.
 *
...
/*!
 * \fn void GXPokeDither(u8 dither)
 * \brief Enables dithering when writing the Embedded Frame Buffer (EFB) using GXPoke*() functions.
 *
 * \note The \a dither enable is only valid when the pixel format (see GXSetPixelFmt()) is either <tt>GX_PF_RGBA6_Z24</tt> or <tt>GX_PF_RGB565_Z16</tt>.<br><br>
 *
...
/*!
 * \fn void GXPokeBlendMode(u8 type,u8 src_fact,u8 dst_fact,u8 op)
 ...
 *
 * \note \a dst_fact can be used only when the frame buffer has <tt>GX_PF_RGBA6_Z24</tt> as the pixel format (see GXSetPixelFmt()).<br><br>
 *
 * \note When type is set to <tt>GX_BM_LOGIC</tt>, the source and EFB pixels are blended using logical bitwise operations.<br><br>
 *
 * \note This function does not effect the normal rendering state; see GXSetBlendMode().
 *
 * \param[in] type \ref blendmode
...
/*!
 * \fn void GXPokeAlphaRead(u8 mode)
 * \brief Determines what value of alpha will be read from the Embedded Frame Buffer (EFB).
 *
 * \details The mode only applies to GXPeek*() functions.
 *
 * \note This feature works no matter what pixel type (see GXSetPixelFmt()) you are using. If you are using the EFB with alpha plane, it is
 ...
 * \return none
 */
void GXPokeAlphaRead(u8 mode);

/*!
 * \fn void GXPokeDstAlpha(u8 enable,u8 a)
 * \brief Sets a constant alpha value for writing to the Embedded Frame Buffer (EFB).
 *
 * \details The EFB pixel type must have an alpha channel for this function to be effective (see GXSetPixelFmt()). The blending operations (see
 * GXPokeBlendMode()) still use source alpha but when writing the pixel color, the constant \a a will replace the pixel alpha in the EFB.
 *
 ...
 * \return none
 */
void GXPokeDstAlpha(u8 enable,u8 a);

/*!
 * \fn void GXPokeARGB(u16 x,u16 y,GXColor color)
 * \brief Allows the CPU to write \a color directly to the Embedded Frame Buffer (EFB) at position \a x,\a y.
 *
 * \details The alpha value in \a color can be compared with the current alpha threshold (see GXPokeAlphaMode()). The color will be blended
 * into the EFB using the blend mode set by GXPokeBlendMode().
 *
...
/*!
 * \fn void GXPeekARGB(u16 x,u16 y,GXColor *color)
...
/*!
 * \fn void GXPokeZ(u16 x,u16 y,u32 z)
 * \brief Allows the CPU to write a z value directly to the Embedded Frame Buffer (EFB) at position \a x,\a y.
 *
 * \details The \a z value can be compared with the current contents of the EFB. The Z compare fuction is set using GXPokeZMode().
 *
...
/*!
 * \fn void GXPeekZ(u16 x,u16 y,u32 *z)
...
/*!
 * \fn void GXPokeZMode(u8 comp_enable,u8 func,u8 update_enable)
 ...
 *
 * \note The normal rendering Z mode (set by GXSetZMode()) is not affected by this function.<br><br>
 *
...
/*!
 * \fn u32 GXGetTexObjFmt(GXTexObj *obj)
 * \brief Returns the texture format described by texture object \a obj.
 *
 * \note Use GXInitTexObj() or GXInitTexObjCI() to initialize the texture format.
 *
 * \param[in] obj ptr to a texture object
 *
 * \return texture format of the given texture object
 */
u32 GXGetTexObjFmt(GXTexObj *obj);

/*!
 * \fn u32 GXGetTexObjMipMap(GXTexObj *obj)
 * \brief Returns the texture mipmap enable described by texture object \a obj.
 * 
 * \note Use GXInitTexObj() or GXInitTexObjCI() to initialize the texture mipmap enable.
 *
 * \param[in] obj ptr to a texture object
 *
 * \return mipmap enable flag
 */
u32 GXGetTexObjMipMap(GXTexObj *obj);

/*!
 * \fn void* GXGetTexObjUserData(GXTexObj *obj)
 * \brief Used to get a pointer to user data from the \ref GXTexObj structure.
 *
 * \details You can use this function to retrieve private data structures from the texture object. This pointer is set using GXInitTexObjUserData().
 *
 ...
 * along with memalign() to allocate texture buffers (see GXCopyTex()).
 *
 * \param[in] wd width of the texture in texels
 * \param[in] ht height of the texture in texels
 * \param[in] fmt format of the texture; use GXTexFmt() or GXCITexFmt() to get it
 ...
 *
 * \note Preloaded textures (see GXPreloadEntireTexture()) are not affected.
 *
 * \return none
 */
...
/*!
 * \fn void GXInvalidateTexRegion(GXTexRegion *region)
 ...
 * GXPreloadEntireTexture()) do not use the tags.<br><br>
 *
...
/*!
 * \fn void GXInitTexCacheRegion(GXTexRegion *region,u8 is32bmipmap,u32 tmem_even,u8 size_even,u32 tmem_odd,u8 size_odd)
 ...
 *
 * \note For pre-loaded textures, the region must be defined by using GXInitTexPreloadRegion().<br><br>
 *
 * \note GXInit() creates default texture regions, so it is not necessary for the application to use this function unless a different Texture Memory
 * configuration is desired. In that case, the application should also define a region allocator using GXSetTexRegionCallback().<br><br>
 *
 * \note The function GXInvalidateTexRegion() can be used to force the texture in main memory associated with this region to be reloaded. This will be
 * necessary whenever the texture data in main memory changes. You may invalidate all cached regions at once using GXInvalidateTexAll().
 *
 ...
 * \return none
 */
void GXInitTexCacheRegion(GXTexRegion *region,u8 is32bmipmap,u32 tmem_even,u8 size_even,u32 tmem_odd,u8 size_odd);

/*!
 * \fn void GXInitTexPreloadRegion(GXTexRegion *region,u32 tmem_even,u32 size_even,u32 tmem_odd,u32 size_odd)
 ...
 * by using GXInitTexCacheRegion(). For pre-loaded textures, the size of the region must match the size of the texture. An application can
 ...
 *
 * \warning GXInit() creates no region for preloading, so the application should allocate appropriate regions if preloading is necessary. It
 * is also required to create cache regions and its allocator by using GXInitTexCacheRegion() and GXSetTexRegionCallback(), otherwise new
...
/*!
 * \fn void GXInitTexObj(GXTexObj *obj,void *img_ptr,u16 wd,u16 ht,u8 fmt,u8 wrap_s,u8 wrap_t,u8 mipmap)
 ...
 * one of eight active texture IDs using GXLoadTexObj().
 *
 * \note To initialize a texture object for color index format textures, use GXInitTexObjCI().<br><br>
 *
 ...
 * is not a mipmap and the texture will be bilerped. To override the filter modes and other mipmap controls, see GXInitTexObjLOD().
 *
...
/*!
 * \fn void GXInitTexObjCI(GXTexObj *obj,void *img_ptr,u16 wd,u16 ht,u8 fmt,u8 wrap_s,u8 wrap_t,u8 mipmap,u32 tlut_name)
 ...
 * and the texture will be bilerped. To override the filter modes and other mipmap controls, use GXInitTexObjLOD(). Mipmap textures should
 ...
 * \return none
 */
void GXInitTexObjCI(GXTexObj *obj,void *img_ptr,u16 wd,u16 ht,u8 fmt,u8 wrap_s,u8 wrap_t,u8 mipmap,u32 tlut_name);
...
/*!
 * \fn void GXInitTexObjData(GXTexObj *obj,void *img_ptr)
 ...
 * GXInitTexObj() or GXInitTexObjCI().
 *
 * \param[in] obj ptr to a texture object
...
/*!
 * \fn void GXInitTexObjWrapMode(GXTexObj *obj,u8 wrap_s,u8 wrap_t)
...
/*!
 * \fn void GXInitTexObjFilterMode(GXTexObj *obj,u8 minfilt,u8 magfilt)
...
/*!
 * \fn void GXInitTexObjMinLOD(GXTexObj *obj,f32 minlod)
...
/*!
 * \fn void GXInitTexObjLODBias(GXTexObj *obj,f32 lodbias)
 ...
 * result is clamped between the values given to GXInitTexObjMinLOD() and GXInitTexObjMaxLOD(). If \a GX_ENABLE is given to
 * GXInitTexObjBiasClamp(), the effect of \a lodbias will diminish as the polygon becomes more perpendicular to the view direction.
 *
...
/*!
 * \fn void GXInitTexObjBiasClamp(GXTexObj *obj,u8 biasclamp)
 * \brief Enables bias clamping for texture LOD.
 *
 * \details If \a biasclamp is \a GX_ENABLE, the sum of LOD and \a lodbias (given in GXInitTexObjLODBias()) is clamped so that it is never
...
/*!
 * \fn void GXInitTexObjEdgeLOD(GXTexObj *obj,u8 edgelod)
 ...
 * for GXInitTexObjMaxAniso() argument).
 *
...
/*!
 * \fn void GXInitTexObjMaxAniso(GXTexObj *obj,u8 maxaniso)
 ...
 * \a maxaniso to \a GX_ANISO_2 or \a GX_ANISO_4, the \a minfilt parameter given to GXInitTexObjFilterMode() should be set to
 * \a GX_LIN_MIP_LIN.
 *
...
/*!
 * \fn GXInitTexObjUserData(GXTexObj *obj,void *userdata)
 * \brief Used to set a pointer to user data in the \ref GXTexObj structure.
 *
 * \details You can use this function to attach private data structures to the texture object. This pointer can be retrieved using GXGetTexObjUserData().
 *
 * \param[in] obj ptr to a texture object
...
/*!
 * \fn void GXLoadTexObj(GXTexObj *obj,u8 mapid)
 * \brief Loads the state describing a texture into one of eight hardware register sets.
 *
 * \details Before this happens, the texture object \a obj should be initialized using GXInitTexObj() or GXInitTexObjCI(). The \a id parameter refers to
 * the texture state register set. Once loaded, the texture can be used in any Texture Environment (TEV) stage using GXSetTevOrder().
 *
 * \note This function will call the functions set by GXSetTexRegionCallback() (and GXSetTlutRegionCallback() if the texture is color-index
 * format) to obtain the texture regions associated with this texture object. These callbacks are set to default functions by GXInit().
 *
 * \warning If the texture is a color-index texture, you <b>must</b> load the associated TLUT (using GXLoadTlut()) before calling GXLoadTexObj().
 *
 * \param[in] obj ptr to a texture object
...
/*!
 * \fn void GXLoadTlut(GXTlutObj *obj,u32 tlut_name)
 * \brief Copies a Texture Look-Up Table (TLUT) from main memory to Texture Memory (TMEM).
 *
 * \details The \a tlut_name parameter is the name of a pre-allocated area of TMEM. The callback function set by GXSetTlutRegionCallback() converts
 ...
 * location of the TLUT in main memory, the TLUT format, and the TLUT size. \a obj should have been previously initialized using GXInitTlutObj().
 *
 * \note GXInit() sets a default callback to convert \a tlut_names from \ref tlutname to \ref GXTlutRegion pointers. The default configuration of
 ...
 * GXInitTlutRegion() and GXInitTexCacheRegion() to allocate TMEM. Then you can define your own region allocation scheme using GXSetTlutRegionCallback()
 * and GXSetTexRegionCallback().
 *
...
/*!
 * \fn void GXLoadTexObjPreloaded(GXTexObj *obj,GXTexRegion *region,u8 mapid)
 * \brief Loads the state describing a preloaded texture into one of eight hardware register sets.
 *
 * \details Before this happens, the texture object \a obj should be initialized using GXInitTexObj() or GXInitTexObjCI(). The \a mapid parameter refers to
 * the texture state register set. The texture should be loaded beforehand using GXPreloadEntireTexture(). Once loaded, the texture can be used in any Texture Environment
 * (TEV) stage using GXSetTevOrder().
 *
 * \note GXInit() initially calls GXSetTevOrder() to make a simple texture pipeline that associates <tt>GX_TEXMAP0</tt> with <tt>GX_TEVSTAGE0</tt>,
 * <tt>GX_TEXMAP1</tt> with <tt>GX_TEVSTAGE1</tt>, etc.<br><br>
 *
 * \note GXLoadTexObjPreloaded() will not call the functions set by GXSetTexRegionCallback() (and GXSetTlutRegionCallback() if the texture is color
 ...
 * callbacks set by GXInit() assume there are no preloaded regions.
 *
 * \param[in] obj ptr to a texture object
...
/*!
 * \fn void GXPreloadEntireTexture(GXTexObj *obj,GXTexRegion *region)
 ...
 * texture region must be the same size as the texture (see GXInitTexPreloadRegion()).
 *
 ...
 * call GXLoadTexObjPreloaded(). The default configuration (as set by GXInit()) of texture memory has no preloaded regions, so you must install
 * your own region allocator callbacks using GXSetTexRegionCallback() and GXSetTlutRegionCallback().
 *
...
/*!
 * \fn void GXInitTlutObj(GXTlutObj *obj,void *lut,u8 fmt,u16 entries)
 ...
 * memory described by this object can be loaded into a TLUT allocated in the texture memory using the GXLoadTlut() function.
 *
...
/*!
 * \fn void GXInitTlutRegion(GXTlutRegion *region,u32 tmem_addr,u8 tlut_sz)
 * \brief Initializes a Texture Look-Up Table (TLUT) region object.
 *
 * \note GXInit() creates default TLUT regions, so the application does not need to call this function unless a new configuration
 * of Texture Memory is desired. In that case, the application should also set a new TLUT region allocator using GXSetTlutRegionCallback().
 *
...
/*!
 * \fn void GXInitTexObjLOD(GXTexObj *obj,u8 minfilt,u8 magfilt,f32 minlod,f32 maxlod,f32 lodbias,u8 biasclamp,u8 edgelod,u8 maxaniso)
 * \brief Sets texture Level Of Detail (LOD) controls explicitly for a texture object.
 *
 * \details It is the application's responsibility to provide memory for a texture object. When initializing a texture object using GXInitTexObj()
 * or GXInitTexObjCI(), this information is set to default values based on the mipmap flag. This function allows the programmer to override those
 * defaults.
 *
 * \note This function should be called after GXInitTexObj() or GXInitTexObjCI() for a particular texture object.<br><br>
 *
 ...
 * \return none
 */
void GXInitTexObjLOD(GXTexObj *obj,u8 minfilt,u8 magfilt,f32 minlod,f32 maxlod,f32 lodbias,u8 biasclamp,u8 edgelod,u8 maxaniso);
 ...
 * according to the texture map that is associated with the texcoord by GXSetTevOrder(). However, there are certain cases where a different scale value is
 * desirable. One such case is when using indirect tiled textures (see GXSetTevIndTile()).
 *
...
/*!
 * \fn void GXSetTexCoordBias(u8 texcoord,u8 s_enable,u8 t_enable)
 ...
 *
 * \note Texture coordinate range bias is something that is normally set automatically by the GX API (during GXBegin()); however, when a texture
 * coordinate is being scaled manually (by using GXSetTexCoordScaleManually()), the associated bias is no longer modified by GX. Thus,
 * GXSetTexCoordBias() allows the bias to be changed while a texture coordinate is being manually controlled.
 *
...
/*!
 * \fn GXTexRegionCallback GXSetTexRegionCallback(GXTexRegionCallback cb)
 * \brief Sets the callback function called by GXLoadTexObj() to obtain an available texture region.
 *
 * \details GXInit() calls this function to set a default region-assignment policy. A programmer can override this default region assignment
 * by implementing his own callback function. A pointer to the texture object and the texture map ID that are passed
 * to GXLoadTexObj() are provided to the callback function.
 *
...
/*!
 * \fn GXTlutRegionCallback GXSetTlutRegionCallback(GXTlutRegionCallback cb)
 * \brief Sets the callback function called by GXLoadTlut() to find the region into which to load the TLUT.
 *
 * \details GXLoadTexObj() will also call \a cb to obtain the Texture Look-up Table (TLUT) region when the texture forma
 * is color-index.
 *
 * GXInit() calls GXSetTlutRegionCallback() to set a default TLUT index-to-region mapping. The name for the TLUT from the texture
 ...
 * GXLoadTlut() will initialize data into the \ref GXTlutRegion which GXLoadTexObj() will subsequently use.
 *
...
/*!
 * \fn void GXInitLightPos(GXLightObj *lit_obj,f32 x,f32 y,f32 z)
 ...
 * load a light object into a hardware light, use GXLoadLightObj() or GXLoadLightObjIdx().
 *
...
/*!
 * \fn void GXInitLightColor(GXLightObj *lit_obj,GXColor col)
 ...
 *
 * \details This direction is used when the light object is used as spotlight or a specular light (see the <i>attn_fn</i> parameter of GXSetChanCtrl()).
 *
 ...
 * position very far from every objects to be lit. (See GXInitLightPos() and GXSetChanCtrl())<br><br>
 *
 ...
 * object into a hardware light, use GXLoadLightObj() or GXLoadLightObjIdx().
 *
 * \param[in] lit_obj ptr to the light object
...
/*!
 * \fn void GXLoadLightObj(GXLightObj *lit_obj,u8 lit_id)
 ...
 *
 * \note The light object must have been initialized first using the necessary GXInitLight*() functions.<br><br>
 *
 * \note Another way to load a light object is with GXLoadLightObjIdx().
 *
...
/*!
 * \fn void GXLoadLightObjIdx(u32 litobjidx,u8 litid)
 ...
 * application is responsible for storing the light object data from the CPU cache (using DCStoreRange()) before calling GXLoadLightObjIdx().
 *
...
/*!
 * \fn void GXInitLightDistAttn(GXLightObj *lit_obj,f32 ref_dist,f32 ref_brite,u8 dist_fn)
 * \brief Sets coefficients for distance attenuation in a light object.
 *
 * \details This function uses three easy-to-control parameters instead of <i>k0</i>, <i>k1</i>, and <i>k2</i> in GXInitLightAttn().
 *
 ...
 *
 * \note If you want more flexible control, it is better to use GXInitLightAttn() and calculate appropriate coefficients.<br><br>
 *
 ...
 * GXInitLightSpot() or GXInitLightAttnA().<br><br>
 *
 * \note This function does not load any hardware registers directly. To load a light object into a hardware light, use GXLoadLightObj() or
 * GXLoadLightObjIdx().
 *
...
/*!
 * \fn void GXInitLightAttn(GXLightObj *lit_obj,f32 a0,f32 a1,f32 a2,f32 k0,f32 k1,f32 k2)
 ...
 * (see GXSetChanCtrl()).
 *
 * \note The convenience function GXInitLightSpot() can be used to set the angle attenuation coefficents based on several spot light
 * types. The convenience function GXInitLightDistAttn() can be used to set the distance attenuation coefficients using one of several
 * common attenuation functions.<br><br>
 *
 * \note The convenience macro GXInitLightShininess() can be used to set the attenuation parameters for specular lights.<br><br>
 *
 ...
 * eye-space vertex normal and the half-angle vector set by GXInitSpecularDir().<br><br>
 *
 * \note This function does not load any hardware registers directly. To load a light object into a hardware light, use GXLoadLightObj()
 * or GXLoadLightObjIdx().
 *
 * \param[in] lit_obj ptr to a light object
 ...
 * from the light position to the vertex. The \a k0-\a 2 coefficients can be set using GXInitLightAttnK(). You can set both the \a a0-\a 2 and \a k0-\a 2 coefficients
 * can be set using GXInitLightAttn(). The light color will be multiplied by the <i>atten</i> factor when the attenuation function for the color channel
 * referencing this light is set to <tt>GX_AF_SPOT</tt> (see GXSetChanCtrl()).
 *
 * \note The convenience function GXInitLightSpot() can be used to set the angle attenuation coefficents based on several spot light types. The
 * convenience function GXInitLightDistAttn() can be used to set the distance attenuation coefficients using one of several common attenuation functions.<br><br>
 *
 * \note This function does not load any hardware registers directly. To load a light object into a hardware light, use GXLoadLightObj() or GXLoadLightObjIdx().
 *
 * \param[in] lit_obj ptr to a light object
 * \param[in] a0 angle attenuation coefficient
 * \param[in] a1 angle attenuation coefficient
...
/*!
 * \fn void GXInitLightAttnK(GXLightObj *lit_obj,f32 k0,f32 k1,f32 k2)
 ...
 *
 * where <i>cos(theta)</i> is the cosine of the angle between the light normal and the vector from the light position to the vertex, and <i>d</i> is the distance
 * from the light position to the vertex. The \a a0-\a 2 coefficients can be set using GXInitLightAttnA(). You can set both the \a a0-\a 2 and \a k0-\a 2 coefficients
 * can be set using GXInitLightAttn(). The light color will be multiplied by the <i>atten</i> factor when the attenuation function for the color channel
 * referencing this light is set to <tt>GX_AF_SPOT</tt> (see GXSetChanCtrl()).
 *
 * \note The convenience function GXInitLightSpot() can be used to set the angle attenuation coefficents based on several spot light types. The convenience
 * function GXInitLightDistAttn() can be used to set the distance attenuation coefficients using one of several common attenuation functions.<br><br>
 *
 * \note Note that this function does not load any hardware registers directly. To load a light object into a hardware light, use GXLoadLightObj() or
 * GXLoadLightObjIdx().
 *
 * \param[in] lit_obj ptr to a light object
 * \param[in] k0 distance attenuation coefficient
...
/*!
 * \fn void GXInitSpecularDirHA(GXLightObj *lit_obj,f32 nx,f32 ny,f32 nz,f32 hx,f32 hy,f32 hz)
 * \brief Sets the direction and half-angle vector of a specular light in the light object.
 *
 * \details These vectors are used when the light object is used only as specular light. In contrast to GXInitSpecularDir(),
 ...
 *
 * \note This function does not load any hardware registers. To load a light object into a hardware light, use GXLoadLightObj()
 * or GXLoadLightObjIdx().<br><br>
 *
 * \note Other notes are similar to those described in GXInitSpecularDir().
 *
 * \param[in] lit_obj ptr to a light object
 * \param[in] nx X coordinate of the light normal
 * \param[in] ny Y coordinate of the light normal
 ...
 * GXSetChanCtrl() by setting the \ref attenfunc to <tt>GX_AF_SPEC</tt>. Furthermore, one must not use GXInitLightDir() or
 * GXInitLightPos() to set up a light object which will be used as a specular light since these functions will destroy the information
 * set by GXInitSpecularDir(). In contrast to diffuse lights (including spotlights) that are considered local lights, a specular light
...
/*!
 * \fn void GXInitLightSpot(GXLightObj *lit_obj,f32 cut_off,u8 spotfn)
 * \brief Sets coefficients for angular (spotlight) attenuation in light object.
 *
 * \details This function uses two easy-to-control parameters instead of \a a0, \a a1, and \a a2 on GXInitLightAttn().
 *
 ...
 * the light direction given by GXInitLightDir() is smaller than this cutoff angle. The value for \a cut_off should be within 0 < \a cut_off <= 90.0, otherwise
 ...
 * color channel setting is using <tt>GX_AF_SPOT</tt> (see GXSetChanCtrl()).
 *
 * \note This function can generate only some kind of simple spotlights. If you want more flexible control, it is better to use GXInitLightAttn() and calculate
 * appropriate coefficients.<br><br>
 *
 * \note This function sets parameters only for angular attenuation. Parameters for distance attenuation should be set by using GXInitLightDistAttn() or
 * GXInitLightAttnK().<br><br>
 *
 * \note This function does not load any hardware registers directly. To load a light object into a hardware light, use GXLoadLightObj() or GXLoadLightObjIdx().
 *
 * \param[in] lit_obj ptr to a light object
 ...
 * the GX thread is the thread that invoked GXInit(); however, it may be changed by calling GXSetCurrentGXThread().
 *
 ...
 */
//lwp_t GXGetCurrentGXThread();

/*!
 * \fn lwp_t GXSetCurrentGXThread()
 ...
 * the thread that invoked GXInit(); however, it may be changed by calling this function.
 *
 ...
 */
//lwp_t GXSetCurrentGXThread();

/*!
 * \fn void GXRestoreWriteGatherPipe()
 * \brief Restores the write-gather pipe.
 *
 * \details The CPU fifo that was attached at the time GXRedirectWriteGatherPipe() was called will be re-attached. If there is data pending
 ...
 * flushed out.
 *
 * \warning This function must be called between successive calls to GXRedirectWriteGatherPipe().
 *
 * \return none
 */
...
/*!
 * \fn void GXSetGPMetric(u32 perf0,u32 perf1)
 ...
 * for the first call to GXReadGPMetric().
 *
 ...
 *
 * \note GXReadGPMetric() and GXClearGPMetric() can be used in the callback associated with the draw sync interrupt (see GXSetDrawSyncCallback()).
 ...
 * also be necessary to send a draw sync token using GXSetDrawSync() or call GXSetDrawDone() after GXReadGPMetric() to ensure that the
...
/*!
 * \fn void GXClearGPMetric()
 * \brief Clears the two virtual GP performance counters to zero.
 *
 * \note The counter's function is set using GXSetGPMetric(); the counter's value is read using GXReadGPMetric(). Consult these for more details.
 *
...
/*!
 * \fn void GXReadXfRasMetric(u32 *xfwaitin,u32 *xfwaitout,u32 *rasbusy,u32 *clks)
 ...
 *
 * \note To set the metric for the counter, call GXSetVCacheMetric(); to read the counter value, call GXReadVCacheMetric().
 *
 * \return none
 */
...
/*!
 * \fn void GXReadVCacheMetric(u32 *check,u32 *miss,u32 *stall)
 * \brief Returns Vertex Cache performance counters.
 *
 * \details Each call to this function resets the counter to zero. GXSetVCacheMetric() sets the metric to be measured by
...
/*!
 * \fn void GXSetVCacheMetric(u32 attr)
 ...
 *
 * \note To clear the counter, call GXClearVCacheMetric(); to read the counter value, call GXReadVCacheMetric().
 *
...
/*!
 * \fn void GXGetGPStatus(u8 *overhi,u8 *underlow,u8 *readIdle,u8 *cmdIdle,u8 *brkpt)
 ...
 * GXDisableBreakPt()). A callback can also be used to notify your application that the break point has been reached. (see
 ...
 *
 * \note The performance metrics can be set using GXSetGPMetric(); the counters can be cleared using GXClearGPMetric().<br><br>
 *
 * \note GXReadGPMetric() and GXClearGPMetric() can be used in the callback associated with the draw sync interrupt (see GXSetDrawSyncCallback()).
 * The function GXSetGPMetric() should <b>not</b> be used in the draw sync callback because it will insert tokens in the GP command stream at random times.<br><br>
 *
 ...
 * may also be necessary to send a draw sync token using GXSetDrawSync() or GXSetDrawDone() before GXReadGPMetric() is called to ensure that the
 * state has actually been processed by the GP.
 *
...
/*!
 * \fn void GXReadBoundingBox(u16 *top,u16 *bottom,u16 *left,u16 *right)
 * \brief Returns the bounding box of pixel coordinates that are drawn in the Embedded Framebuffer (EFB).
 *
 * \details This function reads the bounding box values. GXClearBoundingBox() can be used reset the values of the bounding box.
 *
...
/*!
 * \fn volatile void* GXRedirectWriteGatherPipe(void *ptr)
 ...
 * gather pipe can be restored by calling GXRestoreWriteGatherPipe(). This function cannot be called between a
 * GXBegin()/GXEnd() pair.
 *
 ...
 * GXSetCPUFifo(), GXInitFifoBase(), etc. This function performs very light weight state saves by assuming that the CPU and
 * GP FIFOs never change.
 *
 * \warning <b>No GX commands can be called until the write gather pipe is restored. You MUST call
 * GXRestoreWriteGatherPipe() before calling this function again, or else the final call to restore the pipe will fail.</b>
 *
 ...
 * light object into a hardware light, use GXLoadLightObj() or GXLoadLightObjIdx().<br><br>
 *
 * \note The coordinate space of the light normal should be consistent with a vertex normal transformed by a normal matrix; i.e., it should be
 * transformed to view space.<br><br>
 *
 ...
 * GXSetChanCtrl() by setting \a attn_fn to <tt>GX_AF_SPEC</tt>. Furthermore, one must not use GXInitLightDir() or GXInitLightPos() to
 * set up a light object which will be used as a specular light since these functions will destroy the information set by GXInitSpecularDir().
 ...
 * light object into a hardware light, use GXLoadLightObj() or GXLoadLightObjIdx().<br><br>
 *
 * \note In contrast to GXInitSpecularDirv(), which caclulates half-angle vector automatically by assuming the view vector as (0,0,1), this
 ...
 * highlights.<br><br>
 *
 * \note Other notes are similar to that described in GXInitSpecularDirv().
 *
 * \param[in] lo ptr to the light object
 ...
 * parameters (see GXInitLightAttn()). Note that the equation is attempting to approximate the function (N*H)^shininess. Since the attenuation
 ...
 * attenuation parameter of the lighting channel to <tt>GX_AF_SPEC</tt> using GXSetChanCtrl().
 *
